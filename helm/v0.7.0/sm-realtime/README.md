...
![Version: 0.7.0](https://img.shields.io/badge/Version-0.7.0-informational?style=flat-square) ![AppVersion: 1.36.17-1329687](https://img.shields.io/badge/AppVersion-1.36.17--1329687-informational?style=flat-square)
...
| file://../sm-resource-manager | resourceManager(sm-resource-manager) | ~1.8.0 |
| file://../sm-transcriber | transcribers(sm-transcriber) | ~0.8.0 |
...
| global.flow.image.tag | string | `"0.27.7-1327988"` | Image tag for Flow service component |
...
| global.proxy.image.tag | string | `"2.11.0-1330588"` | Image tag for proxy component |
| global.resourceManager | object | `{"image":{"registry":"speechmaticspublic.azurecr.io","tag":"1.36.17-1329687"}}` | Common resource manager components configuration |
| global.resourceManager.image | object | `{"registry":"speechmaticspublic.azurecr.io","tag":"1.36.17-1329687"}` | Resource manager image configuration |
| global.resourceManager.image.registry | string | `"speechmaticspublic.azurecr.io"` | Registry to pull images from |
| global.resourceManager.image.tag | string | `"1.36.17-1329687"` | Version of resource manager components and sidecars (defaults to appVersion) |
...
| global.transcriber | object | `{"image":{"registry":"speechmaticspublic.azurecr.io","tag":"14.4.0"},"languages":["en"]}` | Common transcriber/inference server configuration |
| global.transcriber.image.registry | string | `"speechmaticspublic.azurecr.io"` | Registry to pull images from |
| global.transcriber.image.tag | string | `"14.4.0"` | Image tag for both transcribers and inference server |
...
| inferenceServerEnhancedRecipe1.inferenceSidecar.registerFeatures.modelCosts.ensemble_en | int | `13` | Cost of English ensemble model request  (Recipe 1) |
...
| inferenceServerStandardAll.inferenceSidecar.registerFeatures.modelCosts.audioEventDetection | int | `15` | Cost of an audio_event_detection model request |
...
| proxy.proxy.config.maxChannelsPerSession | int | `2` | Maximum number of channels supported per session |
...
| transcribers.transcriber.env | list | `[{"name":"SM_ON_PREM","value":"true"},{"name":"SM_WEBSOCKET_CONFIG","value":"{\n  \"autoPingTimeout\": 300\n}\n"},{"name":"SM_INFERENCE_RESPONSE_TIMEOUT_MS","value":"3000"}]` | Environment variables to add to transcriber container |
...
| transcribers.transcriber.preWarm.enabled | bool | `true` | Run a pre-warm session on start to improve connection time for initial sessions |
| transcribers.transcriber.preWarm.operatingPoint | string | `"enhanced"` | Operating point used for the preWarm session; possible values are enhanced and standard |
| transcribers.workerProxy.checkForCapacityOnStart | bool | `true` | Check that there is inference server capacity before allowing transcribers to start |
